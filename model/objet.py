...
def obj_requis():
    cur = get_db().cursor()
    cur.execute("SELECT O.id_obj, O.nom_obj, R.quantite, C.id_choix, C.txt_choix FROM objet O JOIN requis R ON O.id_obj=R.id_obj JOIN choix C ON C.id_choix=R.id_choix")
    columns = [desc[0] for desc in cur.description]
    rows = cur.fetchall()
    return [dict(zip(columns, row)) for row in rows]
...
def obj_requis_choix(id_choix):
    cur = get_db().cursor()
    cur.execute("SELECT O.id_obj, O.nom_obj R.quantite FROM objet O JOIN requis R ON O.id_obj=R.id_obj JOIN choix C ON C.id_choix=R.id_choix WHERE C.id_choix=?", (id_choix,))
    columns = [desc[0] for desc in cur.description]
    rows = cur.fetchall()
    return [dict(zip(columns, row)) for row in rows]
